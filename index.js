...
// PLAYER GUARD STATE
// ============================================================
let playerGuardIntervalId = null;   // handle for the pre-join polling setInterval
let occupiedNotificationSent = false; // ensures the "server occupied" webhook fires only ONCE per occupied session
let disconnectedByPlayerDetection = false; // signals end handler to poll instead of reconnect
...
// ============================================================
// PLAYER GUARD — Feature 1: pre-join check & Feature 2: in-game eviction
// ============================================================
const PLAYER_GUARD_POLL_INTERVAL = 30000; // check every 30s while waiting

function startPlayerGuardPolling() {
  if (playerGuardIntervalId) {
    console.log('[PlayerGuard] Polling already active, skipping duplicate start.');
    return;
  }
  ...
  function checkAndJoin() {
    mc.ping(
      ...
      (err, response) => {
        if (err) {
          // Ping failed — server is unreachable or rejecting status probes (common on Aternos).
          // Do NOT join: if the server is down the bot can't connect anyway.
          // Just wait for the next interval tick and try again.
          console.log('[PlayerGuard] Ping error:', err.message, '— retrying in 30s');
          return;
        }
...
function stopPlayerGuardPolling() {
  if (playerGuardIntervalId) {
    ...
    console.log('[PlayerGuard] Pre-join polling stopped.');
  }
}
...
function createBot() {
  ...
  try {
    ...
    bot.once('spawn', () => {
      ...
      // --------------------------------------------------------
      // Feature 2: evict bot the moment any other player joins
      // --------------------------------------------------------
      // Edge-case: a player was already on the server when we spawned
      // (ping + connect window). Check immediately after spawn.
      const existingPlayers = Object.values(bot.players)
        .filter(p => p.username !== bot.username);
      if (existingPlayers.length > 0) {
        const names = existingPlayers.map(p => p.username).join(', ');
        console.log(`[PlayerGuard] Player(s) already online at spawn (${names}) — disconnecting immediately.`);
        if (config.discord && config.discord.enabled) {
          sendDiscordWebhook(
            `🚨 **Player Already Online** — \`${names}\` was on the server when the bot spawned.\nDisconnecting and returning to watch mode.`,
            0xef4444
          );
        }
      ...
      // Watch for players joining while the bot is connected
      bot.on('playerJoined', (player) => {
        if (!botState.connected) return;
        if (player.username === bot.username) return; // ignore ourselves
        console.log(`[PlayerGuard] Player "${player.username}" joined — disconnecting bot.`);
        if (config.discord && config.discord.enabled) {
          sendDiscordWebhook(
            `🚨 **Player Detected** — \`${player.username}\` joined \`${config.server.ip}\`.\nThe bot has disconnected and will rejoin once the server is empty again.`,
            0xef4444 // red
          );
        }
        disconnectedByPlayerDetection = true;
        ...
        clearAllIntervals();
        try { bot.end(); } catch (e) {
          console.log('[PlayerGuard] Error while disconnecting:', e.message);
        }
      });
...
function scheduleReconnect() {
  ...
  reconnectTimeoutId = setTimeout(() => {
    reconnectTimeoutId = null;
    isReconnecting = false;
    startPlayerGuardPolling(); // always gate reconnects through player guard
  }, delay);
}
...
// Feature 1: check for players before the very first join attempt
startPlayerGuardPolling(); // will call createBot() internally once the server is empty
